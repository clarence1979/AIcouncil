...
export function StandaloneLoginForm({ onLogin }: StandaloneLoginFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    localStorage.setItem('auth-username', result.username || username.trim());
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-950 p-4 relative overflow-hidden">
      <div className="absolute inset-0 pointer-events-none">
        <div
          ...
          style={{
            background:
              'radial-gradient(ellipse 80% 60% at 50% 0%, rgba(16,163,127,0.10) 0%, transparent 60%), radial-gradient(ellipse 60% 50% at 0% 100%, rgba(230,126,34,0.08) 0%, transparent 55%), radial-gradient(ellipse 60% 50% at 100% 80%, rgba(66,133,244,0.08) 0%, transparent 55%)',
          }}
        />
        <svg
      ...
      <div className="relative z-10 w-full max-w-sm">
        <div className="bg-gradient-to-br from-gray-900/95 to-gray-800/95 border border-blue-500/30 rounded-2xl shadow-2xl shadow-blue-500/10 p-8">
          <div className="flex flex-col items-center mb-8">
            ...
            <p className="text-blue-300/60 text-sm mt-1">Sign in to continue</p>
          </div>

          {error && (
            <div className="mb-5 p-3 bg-red-900/30 border border-red-500/30 rounded-lg text-red-300 text-sm">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 bg-gray-800/60 border border-blue-500/30 rounded-lg text-blue-100 placeholder-blue-400/40 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all"
              />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-4 py-2.5 pr-10 bg-gray-800/60 border border-blue-500/30 rounded-lg text-blue-100 placeholder-blue-400/40 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all"
                />
                <button
                  type="button"
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full mt-2 px-6 py-3 bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 disabled:from-gray-700 disabled:to-gray-600 disabled:cursor-not-allowed text-white rounded-xl font-semibold shadow-lg shadow-blue-500/20 hover:shadow-blue-500/40 transition-all flex items-center justify-center gap-2"
            >
              {isLoading ? (
