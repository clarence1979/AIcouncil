...
function getAvailableProviders(): Array<{ key: string; label: string; icon: string; color: string }> {
  return Object.entries(AI_PROVIDERS)
    .filter(([key]) => !!getStoredApiKey(key))
    .map(([key, p]) => ({ key, label: p.name, icon: p.icon, color: p.color }));
}
...
function AddSlot({ onAdd, usedSlots }: AddSlotProps) {
  ...
  if (!open) {
    return (
      <button
        ...
        className="flex flex-col items-center justify-center gap-2 p-4 rounded-xl border-2 border-dashed border-blue-500/30 hover:border-blue-400/60 hover:bg-blue-900/20 transition-all text-blue-400/70 hover:text-blue-300 min-h-[120px] disabled:opacity-40 disabled:cursor-not-allowed"
      >
        <Plus size={22} />
  ...
  return (
    <div className="flex flex-col gap-3 p-4 rounded-xl border-2 border-blue-500/50 bg-blue-900/20 min-h-[120px]">
      ...
      {availableProviders.length === 0 ? (
        ...
        <>
          ...
          {selectedProvider && (
            <div className="relative">
              <select
                ...
                className="w-full appearance-none px-3 py-2 pr-8 bg-gray-800/80 border border-blue-500/30 rounded-lg text-blue-100 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none cursor-pointer"
              >
                {AI_PROVIDERS[selectedProvider as keyof typeof AI_PROVIDERS]?.models.map(m => (
          ...
          <button
            ...
            className="w-full py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-600/30 disabled:cursor-not-allowed text-white rounded-lg text-sm font-semibold transition-colors"
          >
            Add
          </button>
        </>
...
function ParticipantSlot({ participant, onUpdate, onRemove, onConfigure }: ParticipantSlotProps) {
  ...
  return (
    <div className="flex flex-col gap-2 p-4 rounded-xl border border-blue-500/20 bg-gray-800/40 backdrop-blur-sm hover:border-blue-500/40 transition-all">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 min-w-0">
          {participant.avatarUrl ? (
            <img src={participant.avatarUrl} alt={displayName} className="w-8 h-8 rounded-full object-cover flex-shrink-0" />
          ) : (
            <span className="text-xl flex-shrink-0">{participant.avatar}</span>
          )}
          {editingName ? (
            <input
              autoFocus
              ...
              className="flex-1 min-w-0 text-sm font-semibold bg-gray-700 text-blue-100 border border-blue-500/50 rounded px-2 py-0.5 outline-none focus:ring-1 focus:ring-blue-500"
            />
          ) : (
            <button
              onClick={() => { setNameValue(participant.customName || ''); setEditingName(true); }}
              className="text-sm font-semibold text-blue-100 hover:text-blue-300 transition-colors truncate max-w-[100px] text-left"
      ...
      <div className="relative">
        <select
          ...
          onChange={e => handleProviderChange(e.target.value)}
          className="w-full appearance-none px-3 py-1.5 pr-7 bg-gray-900/60 border border-blue-500/20 rounded-lg text-blue-200 text-xs focus:ring-1 focus:ring-blue-500 focus:border-blue-500/50 outline-none cursor-pointer"
        >
          {availableProviders.map(p => (
            <option key={p.key} value={p.key}>{p.icon} {p.label}</option>
          ))}
        </select>
        <ChevronDown size={12} className="absolute right-2 top-1/2 -translate-y-1/2 text-blue-400/50 pointer-events-none" />
      </div>
