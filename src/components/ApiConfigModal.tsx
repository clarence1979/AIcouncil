...
import { X, Check, Sparkles, CheckCircle, XCircle } from 'lucide-react';
...
export function ApiConfigModal() {
  ...
  const handleResearchCharacter = async (provider: string) => {
    ...
    if (!apiKey) {
      toast.warning('OpenAI API key is required for character research.');
      return;
    }
          ...
          content: `You are a character research assistant. Return a JSON object with this exact structure:
{
  ...
  "mannerisms": ["mannerism1", "mannerism2"]
      ...
      if (!response.ok) throw new Error('Character research failed');
      ...
      setFormData({
        ...formData,
        [provider]: { ...currentFormData, characterPersona, avatarUrl },
      });
    } catch (error) {
  ...
  const handleTestVoice = async (provider: string, voiceId: string) => {
    ...
    try {
      ...
      audio.onended = () => { URL.revokeObjectURL(audioUrl); setTestingVoice(null); };
      audio.onerror = () => { URL.revokeObjectURL(audioUrl); setTestingVoice(null); };
      await audio.play();
    } catch (error) {
  ...
  const getNextAvailableAvatar = (): Avatar => {
    ...
    return AVAILABLE_AVATARS.find(avatar => !usedAvatars.includes(avatar)) || '🤖';
  };

  const getRandomPersonality = (): Personality =>
    AVAILABLE_PERSONALITIES[Math.floor(Math.random() * AVAILABLE_PERSONALITIES.length)];
  ...
  const handleSave = (provider: string) => {
    ...
    if (!apiKey) {
      toast.error(`No API key configured for ${AI_PROVIDERS[provider as keyof typeof AI_PROVIDERS]?.name}. Please contact your administrator.`);
      return;
    }
  ...
  const getProviderParticipants = (provider: string) =>
    participants.filter(p => p.provider === provider && p.isActive);
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl shadow-2xl border border-blue-500/30 max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between p-6 border-b border-blue-500/20">
          ...
          <button onClick={() => setShowApiConfig(false)} className="text-blue-300 hover:text-blue-100 transition-colors">
            <X size={24} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {Object.entries(AI_PROVIDERS).map(([key, provider], index) => {
            ...
            const hasKey = !!getStoredApiKey(key);

            return (
              <div key={key}>
                {index > 0 && <div className="relative mb-4"><div className="absolute inset-0 flex items-center"><div className="w-full border-t border-blue-500/20"></div></div></div>}

                <div className={`border rounded-xl overflow-hidden transition-all ${hasKey ? 'border-blue-500/20 hover:border-blue-500/40' : 'border-gray-700/40 opacity-60'}`}>
                  <div className="p-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg flex items-center justify-center text-xl" style={{ backgroundColor: provider.color + '20' }}>
                          {provider.icon}
                        </div>
                        <div className="text-left">
                          <h3 className="font-semibold text-blue-100">{provider.name}</h3>
                          <p className="text-sm text-blue-300/60">{provider.models.map(m => m.name).join(', ')}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        {hasKey ? (
                          <span className="text-xs text-emerald-400 flex items-center gap-1">
                            <CheckCircle size={14} /> Key Available
                          </span>
                        ) : (
                    ...
                    {!hasKey && (
                      <div className="mt-3 px-4 py-2 bg-gray-800/40 rounded-lg text-center text-xs text-gray-500">
                        API key not provided by your portal
                      </div>
                    )}
                  </div>
                  ...
                  {expanded && hasKey && (
                    <div className="p-4 bg-gray-800/30 border-t border-blue-500/20 space-y-4">
                      {justSaved === key && (
                        <div className="flex items-center gap-2 p-3 rounded-lg bg-emerald-900/30 text-emerald-300 border border-emerald-500/30">
                          <Check size={18} className="flex-shrink-0" />
                          <span className="text-sm font-semibold">Successfully saved! AI added to your council.</span>
                        </div>
                      )}

                      <div>
                        <label className="block text-sm font-medium text-blue-200 mb-2">Model</label>
                        <select
                          value={data.model || provider.models[0].id}
                          onChange={e => setFormData({ ...formData, [key]: { ...data, model: e.target.value } })}
                      ...
                      <div>
                        <label className="block text-sm font-medium text-blue-200 mb-2">Custom Name (Optional)</label>
                        <input
                          type="text"
                          value={data.customName || ''}
                          onChange={e => setFormData({ ...formData, [key]: { ...data, customName: e.target.value } })}
                      ...
                      <div>
                        <label className="block text-sm font-medium text-blue-200 mb-2">AI Voice (Powered by OpenAI)</label>
                        <div className="grid grid-cols-1 gap-2">
                          {AI_VOICES.map(voice => (
                            <div
                              ...
                              onClick={() => setFormData({ ...formData, [key]: { ...data, voiceName: voice.id } })}
                            >
                              <div className="flex items-center justify-between">
                                <div>
                                ...
                                <button
                                  onClick={e => { e.stopPropagation(); handleTestVoice(key, voice.id); }}
                      ...
                      <div className="border-t border-gray-700/50 pt-4">
                        <label className="block text-sm font-medium text-blue-200 mb-2">Character Persona (Optional)</label>
                        <p className="text-xs text-blue-300/60 mb-3">Give your AI a specific character with auto-generated portrait</p>
                          ...
                          <div className="space-y-2">
                            <div className="flex gap-2">
                              <input
                                ...
                                onChange={e => setCharacterName({ ...characterName, [key]: e.target.value })}
                            ...
                            <p className="text-xs text-blue-300/50">Uses OpenAI to research character traits and generate an AI portrait</p>
                          </div>
                        )}
                          ...
                          <div className="bg-gray-800/50 border border-blue-500/20 rounded-lg p-4 space-y-3">
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1">
                                <h4 className="font-semibold text-blue-100 mb-1">{data.characterPersona.name}</h4>
                                <p className="text-sm text-blue-300/80 mb-2">{data.characterPersona.description}</p>
                            ...
                            <button
                              onClick={() => {
                                setFormData({ ...formData, [key]: { ...data, characterPersona: undefined } });
                                setCharacterName({ ...characterName, [key]: '' });
                              }}
                              className="w-full text-sm text-blue-400 hover:text-blue-200 font-medium"
                            >
                              Remove Character
                            </button>
                          </div>
                      ...
                      <button
                        ...
                        disabled={!data.model || justSaved === key}
                        className="w-full px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                      >
